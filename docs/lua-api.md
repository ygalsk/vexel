...
Virtual resolution: 320×180 default (configurable). 8 compositor layers (0–7), alpha-blended bottom-to-top; text renders above all pixel layers.
...
- [Lifecycle](#lifecycle)
- [Graphics — Text + Primitives](#graphics--text--primitives)
- [Graphics — Images (Immediate)](#graphics--images-immediate-mode)
- [Graphics — ECS Sprites (Retained)](#graphics--ecs-sprites-retained-mode)
- [Graphics — Tilemap](#graphics--tilemap)
- [Scenes](#scenes)
- [Input](#input)
- [Audio](#audio)
- [Timers + Tweens](#timers--tweens)
...
-- Move: just update position
local pos = engine.world.get(e, "position")
engine.world.set(e, "position", { x = pos.x + 10, y = pos.y })

engine.world.remove(e, "animation")  -- sprite stays on current frame
...
| Use case | API |
|----------|-----|
| Persistent animated objects | ECS sprites (auto-animated, auto-rendered) |
...
| Tilemaps | `draw_tilemap` |

### Known limitations

- `visible` on SpriteComp is not settable/gettable from Lua — despawn or move off-screen instead
- `frame` is not returned by `world.get(e, "sprite")` — managed by the animation system
